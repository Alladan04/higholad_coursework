...
***На всех страницах с объявлениями показывается информация о количестве ставок. Возможно имеет смысл в таблицу Auctions записывать агрегат (количество ставок)

## 5. Физическая схема данных
### Выбор СУБД:
В качестве основной базы данных будем использоватьь распределенную СУБД CocroachDB, т.к. она позволяет масшатабироваться "из коробки":
- распределение нагрузки между узлами кластера
- автоматическая репликация
- возможность обеспечить транзакционную целостность данных в распределенной среде
- автоматический шардинг данных

Для сбора статистических данных будем использовать ClickHouse.

Для хранения клиентских сессий и кеширования данных будем использовать Redis Cluster, который включает в себя наборр хостов,
